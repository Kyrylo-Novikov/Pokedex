  ...
  try {
    if (!cacheOverview || cacheOverview.length < currentLoadLimit) {
      let respons = await fetch(
        BASE_URL + path + `?limit=${currentLoadLimit}&offset=${offset}.json`
      );
...
function workWithFoundetPokemon(
  searchResponsToArrayResult,
  searchResultRef,
  inputRef
) {
  rendertPokeID.clear();
  let searchingName = searchResponsToArrayResult.filter((nameToSearch) =>
    nameToSearch.name.toLowerCase().startsWith(inputRef.value.toLowerCase())
  );
  searchingName.slice(0, 10).forEach((searchResult) => {
    let pokeGifUrl = `${BASE_IMG_URL}${pathGif}${searchResult.url
      .replace(/\D+/g, "")
      .slice(1)}.gif`;
    let pokeImgUrl = `${BASE_IMG_URL}${pathImg}${searchResult.url
      .replace(/\D+/g, "")
      .slice(1)}.png`;
    if (searchResult.url.replace(/\D+/g, "").slice(1) <= 1025) {
      searchResultRef.innerHTML += templateSearchin(
        searchResult.name,
        searchResult.url.replace(/\D+/g, "").slice(1),
        pokeImgUrl,
        pokeGifUrl
      );
    }
...
async function pokemonOvInfos() {
  ...
  for (
    let iPokeName = lastUsedIndex;
    iPokeName < responsToArrayResult.length;
    iPokeName++
  ) {
    let pokeRef = responsToArrayResult[iPokeName].url
      .replace(/\D+/g, "")
      .slice(1);
    let pokemonInfosToJson = await fetchOnePokemon(pokeRef);
    if (
      !fetchedPokemon.some(
        (pokemonInfo) => pokemonInfo.name === pokemonInfosToJson.species.name
      )
    ) {
...
async function pokeTypName(
  responsTypesToArray,
  typesOfTheDisplayedPokemon,
  pokemonInfosToJson
) {
...
async function fetchPokeTypImg(pokemonInfosToJson, singelTypesID) {
  try {
    if (typeImgCache[singelTypesID]) {
      return displayPokeTypImg(
        pokemonInfosToJson,
        singelTypesID,
        typeImgCache[singelTypesID]
      );
    }
...
async function setBgColor(pokemonInfosToJson, singelTypesID) {
  let bgColorRef = await waitForElement(
    `bg-${pokemonInfosToJson.species.name}-img`
  );
  if (bgColorRef) {
...
async function transferBgClassToDialog(pokemonName) {
  ...
  let dialogBgColorRef = document.getElementById(
    `lightbox-content${pokemonName}`
  );
...
async function displayPokeTypImg(pokemonInfosToJson, singelTypesID, typImgUrl) {
  let typeOnCardRef = await waitForElement(
    `${pokemonInfosToJson.species.name}-type-area`
  );
  if (typeOnCardRef) {
    let noDoubleTypeDisplay = typeOnCardRef.querySelectorAll(
      `.type-img[id*="-${singelTypesID}-type-img"]`
    );
    if (noDoubleTypeDisplay.length === 0) {
      typeOnCardRef.innerHTML += typeTemplate(
        pokemonInfosToJson,
        typImgUrl,
        singelTypesID
      );
